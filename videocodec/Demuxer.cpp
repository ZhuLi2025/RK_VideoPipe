...
bool Demuxer::open(const std::string &url) {
    ...
    // av_dict_set(&opt, "buffer_size", "10000", 0); //设置缓存大小,1080p可将值跳到最大
    ...
    av_dict_set(&opt, "buffer_size", "1024", 0);
    av_dict_set(&opt, "rtsp_transport", "udp", 0);
    av_dict_set(&opt, "stimeout", "3000000", 0);  // 3s timeout
    av_dict_set(&opt, "max_delay", "0", 0);
    av_dict_set(&opt, "fflags", "nobuffer+flush_packets", 0);
    av_dict_set(&opt, "flags", "low_delay", 0);
    av_dict_set(&opt, "reorder_queue_size", "0", 0);
