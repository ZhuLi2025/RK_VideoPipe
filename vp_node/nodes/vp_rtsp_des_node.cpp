...
namespace vp_nodes {
    ...
    vp_rtsp_des_node::vp_rtsp_des_node(std::string node_name, 
        ...
        if(gst_encoder_name.find("mpph264enc")!=std::string::npos){
            gst_template = vp_utils::string_format(gst_template, gst_encoder_name.c_str(),host_ip.c_str(),base_udp_port + channel_index);
        }
        else{
            gst_template = vp_utils::string_format(gst_template, gst_encoder_name.c_str(), host_ip.c_str(),base_udp_port + channel_index);
        }
