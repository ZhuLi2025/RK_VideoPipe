...
// ===== vp_rtsp_mul_des_node.cpp =====

#include "vp_rtsp_mul_des_node.h"

namespace vp_nodes {
        
    vp_rtsp_mul_des_node::vp_rtsp_mul_des_node(std::string node_name, 
                        int channel_index, 
                        std::string host_ip,
                        int rtsp_port, 
                        ...
                        RTP(base_udp_port + channel_index * 2),
                        RTCP(base_udp_port + channel_index * 2 + 1){
        if (rtsp_name.empty()) {
        ...
        // format gst pipeline template for VideoWriter
        gst_template = vp_utils::string_format(gst_template,
                                               gst_encoder_name.c_str(),
                                               host_ip.c_str(), RTP,
                                               host_ip.c_str(), RTCP);
    ...
    vp_rtsp_mul_des_node::~vp_rtsp_mul_des_node() { 
        deinitialized();
    }
    GstRTSPServer* vp_rtsp_mul_des_node::rtsp_server = NULL;
    std::shared_ptr<vp_objects::vp_meta> vp_rtsp_mul_des_node::handle_frame_meta(std::shared_ptr<vp_objects::vp_frame_meta> meta) {
    ...
    void vp_rtsp_mul_des_node::start_rtsp_streaming () {
        ...
        // use RTP port (RTP) for udpsrc. add rtpjitterbuffer and depay/parse to be robust
        sprintf (udpsrc_pipeline,
                ...
                RTP, (unsigned long)udp_buffer_size);
        ...
        VP_INFO(vp_utils::string_format("[%s] is going to push rtsp stream, please visit:[%s] (RTP=%d RTCP=%d)", node_name.c_str(), to_string().c_str(), RTP, RTCP));
    }

    std::string vp_rtsp_mul_des_node::to_string() {
        return "rtsp://"+host_ip+":" + std::to_string(rtsp_port) + "/" + rtsp_name;
    }
}
